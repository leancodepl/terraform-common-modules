...
resource "azurerm_key_vault_access_policy" "owner_access_to_main_keyvault" {
  ...
  key_permissions         = coalesce(var.owner_access_policy.key_permissions, ["Get", "List", "Update", "Create", "Delete", "Purge", "Recover", "Encrypt", "Decrypt", "WrapKey", "UnwrapKey", "GetRotationPolicy", "SetRotationPolicy"])
